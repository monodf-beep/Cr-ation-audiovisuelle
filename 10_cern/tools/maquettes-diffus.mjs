...
// Aire linguistique : pas de contour. Des taches de lumiere (lon, lat, rayon en px) qui se fondent
// les unes dans les autres et debordent les frontieres administratives. Approximation a la main :
// Savoie, Aoste, Geneve, Vaud, Ain, Valais francophone, nord de l'Isere avec Grenoble, Lyonnais,
...
function page(variante) {
  const couche = variante === 'L'
    ? `<g mask="url(#fondu-l)"><g filter="url(#flou-l)" opacity="0.55">${taches.map(([lon, lat, r]) => { const [x, y] = P([lon, lat]); return `<circle cx="${x}" cy="${y}" r="${r}" fill="url(#tache)"/>`; }).join('')}</g></g>`
    : variante === 'A'
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  <svg viewBox="0 0 1080 1920">
    <defs>
      ...
      <filter id="flou-l" x="-30%" y="-30%" width="160%" height="160%"><feGaussianBlur stdDeviation="45"/></filter>
      <radialGradient id="tache"><stop offset="0" stop-color="#0a36af"/><stop offset="0.4" stop-color="#0a36af" stop-opacity="0.5"/><stop offset="1" stop-color="#0a36af" stop-opacity="0"/></radialGradient>
      <radialGradient id="g-langue" gradientUnits="userSpaceOnUse" cx="${cxL}" cy="${cyL}" r="660"><stop offset="0" stop-color="#fff"/><stop offset="0.35" stop-color="#fff" stop-opacity="0.9"/><stop offset="1" stop-color="#fff" stop-opacity="0"/></radialGradient>
